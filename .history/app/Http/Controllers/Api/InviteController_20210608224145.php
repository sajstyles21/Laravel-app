...
class InviteController extends Controller
{
    ...
    public function getUserData(Request $request)
    {
        try {
            ...
            if ($invite) {
                $user = User::create(['email'=>$invite->email,'name'=>$request->username,'password'=> Hash::make($request->password),'status'=>0]);
                $pin = Str::random(6);
                $data = [
    ...
    public function confirmPin(Request $request)
    {
        try {
            ...
            if ($invite) {
                //create user
                $user = User::where(['email' => $invite->email, 'name' => $request->username, 'password' => Hash::make($request->password)]);
                if ($user) {
